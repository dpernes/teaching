...
void lista_imprime(lista *lst, FILE *file)
{
    if(!lst) return;
    if(lst->tamanho == 0) return;
...
lista *lista_pesquisa_substring(lista *lst, char *substring)
{
    ...
    for(int i=0; i<lst->tamanho; i++)
    {
        ...
        if(strstr(l_elem->str, substring))
            if(!lista_insere(sub_lst, l_elem->str, NULL))
            {
                lista_apaga(sub_lst);
                return NULL;
            }
...
lista* lista_concatena(lista *lst1, lista *lst2)
{
    ...
    {
        l_elem = lista_elemento(lst2, i);
        if(!l_elem) return NULL;

        if(!lista_insere(lst1, l_elem->str, NULL)) return NULL;
    }
...
{
    if(!lst || !inserir) return -1;

    l_elemento* l_elem;
    int cmp, lst_tamanho = lst->tamanho;
    for(int i=0; i<lst->tamanho; i++)
    {
        l_elem = lista_elemento(lst, i);
        if(!l_elem) return -1;

        cmp = strcmp(inserir, l_elem->str);
        if(cmp == 0) return -1;  // o elemento já existe na lista

        else if(cmp < 0)
        {
            if(!lista_insere(lst, inserir, l_elem)) return -1;
            return i;
        }
    }

    if(!lista_insere(lst, inserir, NULL)) return -1;
